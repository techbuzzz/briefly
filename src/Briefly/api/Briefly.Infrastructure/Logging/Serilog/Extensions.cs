...
namespace Briefly.Infrastructure.Logging.Serilog;
            ...
            logger.WriteTo.OpenTelemetry(options =>
            {
                try
                {
                    ...
                    var (otelResourceAttribute, otelResourceAttributeValue) =
                        builder.Configuration["OTEL_RESOURCE_ATTRIBUTES"]?.Split('=') switch
                        {
                            [string k, string v] => (k, v),
                            _ => throw new Exception(
                                $"Invalid header format {builder.Configuration["OTEL_RESOURCE_ATTRIBUTES"]}")
                        };
