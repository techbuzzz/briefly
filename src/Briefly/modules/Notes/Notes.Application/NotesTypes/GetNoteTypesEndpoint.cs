...
public class GetNoteTypesEndpoint(
    ILogger<GetNoteTypesEndpoint> logger,
    [FromKeyedServices(NotesMetadata.DIKey)] IRepository<NoteType> repository) 
    : Endpoint<GetNoteTypesRequest, IPagedList<NoteTypeDto>>
{
    ...
    public override async Task HandleAsync(GetNoteTypesRequest req, CancellationToken ct)
    {
        ...
        var spec = new NoteTypesByPaginationFilterSpec<NoteTypeDto>(req, selector);
