...
public class NoteTypesByPaginationFilterSpec : EntitiesByPaginationFilterSpec<NoteType>
{
        ...
        : base(filter)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filter.NameFilter)) Query.Where(nt => nt.Name.Contains(filter.NameFilter));

        if (filter.ActiveOnly.HasValue && filter.ActiveOnly.Value) Query.Where(nt => nt.IsActive);
